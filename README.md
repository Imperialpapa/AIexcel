...
- **스타일 자동 적용**: 배경색, 글꼴 색상, 굵게, 이탤릭, 정렬 등 엑셀 스타일 보존
- **열 너비 자동 보존**: 엑셀 파일의 열 너비를 그대로 유지
- **행/열 크기 조절**: 열 헤더와 행 헤더의 경계를 드래그하여 크기 조절 (시트별 저장)
...
5. **열 너비 조절**: 열 헤더(A, B, C...) 오른쪽 경계를 드래그하여 너비 조절
6. **행 높이 조절**: 행 헤더(1, 2, 3...) 하단 경계를 드래그하여 높이 조절
7. **AI 패널 크기 조절**: AI 패널 왼쪽 가장자리를 드래그하여 너비 조절
8. **행/열 선택**: 행 번호나 열 문자(A, B, C...)를 클릭하여 전체 행/열 선택
9. **범위 선택**: Shift 키를 누른 채 행/열 헤더를 클릭하여 범위 선택
10. **키보드 네비게이션**: 방향키로 셀 간 이동, Tab/Enter로 빠른 입력
...
### 5. 스타일 및 서식
- 엑셀 파일을 열면 **배경색, 글꼴 색상, 굵게, 이탤릭, 정렬** 등의 스타일이 자동으로 적용됩니다
- F12 콘솔에서 각 시트별 스타일 적용 셀 개수를 확인할 수 있습니다
- **주의**: 기본 SheetJS 라이브러리는 일부 복잡한 스타일을 완전히 읽지 못할 수 있습니다
- 행/열 크기 조절은 시트별로 저장되어 시트 전환 후에도 유지됩니다
...
| **ExcelIO** | 엑셀 파일 가져오기/내보내기, 열 너비 보존 |
